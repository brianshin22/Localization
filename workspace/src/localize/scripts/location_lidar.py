...
#
# ABOUT THIS FILE
# This file is going to be the ROS node executing the particle filter including
# LIDAR measurements. However, it still needs to be implemented.
#
# REVISION HISTORY
# [Apr 03, 2016] created
...
import LPF_functions_lidar_rosnode as pf
import logging
...
activate_LidarMeas = rospy.get_param("/location_sender/activate_LidarMeas")
...
lidar_dX_noise = rospy.get_param("/location_sender/lidar_dX_noise")
lidar_dY_noise = rospy.get_param("/location_sender/lidar_dY_noise")
lidar_dPsi_noise = rospy.get_param("/location_sender/lidar_dPsi_noise")
...
def callback8(ICP_Output):
    global dX_meas, dY_meas, dPsi_meas, Lidar_available
    dX_meas = ICP_Output.x
    dY_meas = ICP_Output.y
    dPsi_meas = ICP_Output.theta
    Lidar_available = 1
...
def location_sender():
    global GPS_count, GPS_available, V_straight_available, del_f_available, V_lateral_available, \
           dot_Psi_available, RightLane_available, LeftLane_available, Lidar_available
    ...
    rospy.Subscriber("ICP_Output", Pose2D, callback8)
    ...
    while not rospy.is_shutdown():
        ...
        (X_estimate, Y_estimate, Psi_estimate) = pf.LPF_main( \
            1.0/frequency, num_particles, GPS_available*GPS_accept[GPS_count], Lidar_available, \
