...
#             a1_LL, V_lateral, dot_Psi, V_lateral_bias, Probability]
#-----------------------------------------------------------------------------
    ...
    particles = np.zeros((11,num_particles))                                        
...
def LPF_propagate(particles, num_particles, dt, V_straight_meas, del_f_meas, \
    ...
    Psi_rel_threshold, Lane_threshold, DynamicModel_threshold, activate_LaneMeas, activate_DynamicModel, \
...
def LPF_resample(particles, num_particles, X_meas, Y_meas, Psi_meas, V_lateral_meas, dot_Psi_meas, \
    a_r0, a_l0, a_r1, a_l1, a_r_quality, a_l_quality, gps_X_noise, gps_Y_noise, gps_Psi_noise, \
    a_r0_noise, a_l0_noise, a_r1_noise, a_l1_noise, gps_V_lateral_noise, dot_Psi_noise, \
    activate_LaneMeas, activate_DynamicModel, GPS_available):
...
#-----------------------------------------------------------------------------
    ...
    particles[:,10] = 1e0 * gps_X_prob * gps_Y_prob * gps_Psi_prob * \
                            a_r0_prob * a_l0_prob * a_r1_prob * a_l1_prob * \
                            V_lateral_prob * dot_Psi_prob;
    
    # compute normalization constant
    norm_constant = sum(particles[:,10])
    ...
    particles[:,10] = particles[:,10]/norm_constant
    ...
    new_particles = np.zeros((num_particles,11))
...
#-----------------------------------------------------------------------------    
    ...
    if GPS_available:
        ...
        new_particles = np.concatenate(([init_X_values],[init_Y_values],[init_Psi_values],
                                ...
                                [particles[through_indices,[9]]],[particles[through_indices,[10]]]),axis=0)
...
def LPF_main(dt, num_particles, GPS_available, \
...
#-----------------------------------------------------------------------------
    ...
    # if access is not successful, initalize the particles
        ...
        # intitialize previous_GPS_missing
