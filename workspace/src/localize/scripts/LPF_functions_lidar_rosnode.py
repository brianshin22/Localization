...
# REVISION HISTORY
# [Apr 03, 2016] created
# [Apr 11, 2016] adapted to accept slower LIDAR rate than PF update rate
#
#-----------------------------------------------------------------------------
...
def LPF_propagate(particles, num_particles, dt, V_straight_meas, del_f_meas, \
    ...
    Psi_rel_threshold, Lane_threshold, Lidar_available, DynamicModel_threshold, \
...
#-----------------------------------------------------------------------------
    ...
    #####################################################################
    ## IF LIDAR IS ACTIVATED AND AVAILABLE, SAVE CURRENT POSE ESTIMATE ##
    #####################################################################
    
    if activate_LidarMeas and Lidar_available:
            
        # safe current estimate of X, Y and Psi in global frame in respective particle state
        particles[:,10] = np.mean(LPF_main.particles[np.isfinite(LPF_main.particles[:,0]),0])*np.ones(num_particles)
        particles[:,11] = np.mean(LPF_main.particles[np.isfinite(LPF_main.particles[:,1]),1])*np.ones(num_particles)
        particles[:,12] = np.mean(LPF_main.particles[np.isfinite(LPF_main.particles[:,2]),2])*np.ones(num_particles)
...
def LPF_propagate_vec(particles, num_particles, dt, V_straight_meas, del_f_meas, \
    ...
    activate_LaneMeas, activate_LidarMeas, Lidar_available, activate_DynamicModel, \
    mass, Izz, lf, lr, B_f, B_r, C_f, C_r, D_f, D_r, E_f, E_r):
...
#-----------------------------------------------------------------------------    
    ...
    if (activate_LaneMeas):
        ...
        if (a_l_quality >= 2):
            ...
            particles[:,6] = np.tan(np.arctan(np.polyval(dpl,Al_x_cand_CLF[index_max]))-Psi_rel_CLF)
...
#-----------------------------------------------------------------------------
    ...
    if (activate_LidarMeas and Lidar_available and GPS_available == 0):
        lidar_dX_prob = norm.pdf(dX_meas*np.ones(num_particles),particles[:,0]-particles[:,10],lidar_dX_noise*np.ones(num_particles))
        lidar_dY_prob = norm.pdf(dY_meas*np.ones(num_particles),particles[:,1]-particles[:,11],lidar_dY_noise*np.ones(num_particles))
        lidar_dPsi_prob = norm.pdf(dPsi_meas*np.ones(num_particles),particles[:,2]-particles[:,12],lidar_dPsi_noise*np.ones(num_particles))
...
def LPF_main(dt, num_particles, GPS_available, Lidar_available, \
...
#-----------------------------------------------------------------------------
    ...
    # try to access LPF_main.particles and if that is not successful
    try:
        dummy = LPF_main.particles[0]
    
    # if access is not successful, initalize the particles
    except:
